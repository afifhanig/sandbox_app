...
import 'package:sandbox_flutter_3/feature/push_notif/push_notif_controller.dart';
import 'package:sandbox_flutter_3/feature/sqflite/database_1/first_database.dart';
import 'package:sandbox_flutter_3/splash_screen.dart';
...
class MyHttpOverrides extends HttpOverrides {
  ...
  HttpClient createHttpClient(SecurityContext? context) {
    return super.createHttpClient(context ?? securityContext)..badCertificateCallback = (X509Certificate cert, String host, int port) => false; // Reject invalid certificates
  }
}
...
void main() async {
  ...
  initFirebase();
  await DatabaseHelper.instance.deleteEntireDatabase();
  await DatabaseHelper.instance.initDB();

  //
  try {
    ByteData data = await rootBundle.load("assets/images/leaf_cert.pem");
    ...
    debugPrint('Succeeded in adding cert');
  } catch (e) {
    debugPrint('Error adding cert: $e');
  }
  runApp(MyApp());
}
...
initFirebase() async {
  ...
  final pushNotifController = PushNotifController(FirebaseMessaging.instance);
  pushNotifController.initialize();
  FirebaseMessaging.onBackgroundMessage(_messageHandler);
}

@pragma('vm:entry-point')
Future<void> _messageHandler(RemoteMessage message) async {
  print('background message1 ${message.notification!.body}');
