...
import 'package:camera/camera.dart';
import 'package:flutter/material.dart';
import 'package:sandbox_flutter_3/feature/camera/basic_camera/image_preview_aspect_ratio.dart';
import 'package:measure_size/measure_size.dart';
//import 'package:flutter_camera_practice/preview_page.dart';

class BasicCameraWithAspectRatio extends StatefulWidget {
  const BasicCameraWithAspectRatio({Key? key, required this.cameras}) : super(key: key);

  final List<CameraDescription>? cameras;

  @override
  State<BasicCameraWithAspectRatio> createState() => _BasicCameraWithAspectRatioState();
}

class _BasicCameraWithAspectRatioState extends State<BasicCameraWithAspectRatio> {
    ...
    try {
      ...
      boxOffsets != null
          ? Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => ImagePreviewAspectRatio(
                  picture: picture,
                  borderSize: boxSize!,
                  borderOffset: boxOffsets!,
                ),
              ),
            )
          : print('tidak ada box offset');
    } on CameraException catch (e) {
  ...
  Widget cam(CameraController controller) {
    // double previewAspectRatio = 0.7;
    ...
    if (controller == null || !controller.value.isInitialized) {
      return Text('No Camera to Preview');
    }
